...
3. Processing and cleaning the data using Apache Spark on Dataproc.

4. Building structured, partitioned, and clustered tables in BigQuery for efficient querying.

5. Modeling the data with dbt into dimension and fact tables (dim_items, dim_users, dim_time, fct_reviews).

6. Visualizing insights through an interactive dashboard in Power BI.

Here are the key data entities processed:

1. Reviews: Review ID, ratings, verified purchase status, timestamps, helpful votes

2. Users: Review activity frequency, total spending, review counts

3. Products: Categories, prices, average ratings
